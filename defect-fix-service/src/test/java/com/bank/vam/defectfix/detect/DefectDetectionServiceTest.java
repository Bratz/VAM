...
class DefectDetectionServiceTest {
    ...
    void onlyFilesDefectsThatAreNewComparedToTheMergeBase() throws Exception {
        ...
        service.handleFailedPrRun(DefectDetectionService.Stack.FRONTEND, "base1", "head1", "some-branch", 42, 0);

        ArgumentCaptor<DetectedDefect> filed = ArgumentCaptor.forClass(DetectedDefect.class);
        verify(ticketService, times(1)).fileIfNew(filed.capture(), eq(42), eq("some-branch"), eq(0));
    ...
    void noBaselineArtifactMeansEverythingAtHeadIsTreatedAsNew() throws Exception {
        ...
        service.handleFailedPrRun(DefectDetectionService.Stack.FRONTEND, "base1", "head1", "some-branch", 7, 0);

        verify(ticketService, times(1)).fileIfNew(any(), eq(7), eq("some-branch"), eq(0));
    }
}

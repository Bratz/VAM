  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-warning-200 dark:border-warning-800/60">
        ...
        <span className="text-caption font-semibold uppercase tracking-wider text-warning-700 dark:text-warning-200">
          Action requires confirmation
        </span>
        <span className="ml-auto text-caption text-warning-700/70 dark:text-warning-300/70 flex items-center gap-1">
      ...
      <div className="px-3.5 py-3 space-y-2">
        <h4 className="font-display body-strong font-semibold">
          {action.title}
        </h4>
        <p className="text-caption text-neutral-700 dark:text-neutral-300 leading-relaxed">
          {action.description}
        </p>
...
const ParamsTable: React.FC<{ params: Record<string, unknown> }> = ({ params }) => {
  ...
  return (
    <div className="rounded-md bg-white/70 dark:bg-primary-950/40 border border-warning-100/80 dark:border-warning-800/40 p-2">
      <div className="flex items-center gap-1.5 mb-1 caption">
        <ListTree className="w-3 h-3" /> Parameters
      </div>
      <div className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-caption font-mono">
        {entries.map(([k, v]) => (

    ...
    <div className="space-y-3">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 border border-edge-strong rounded-lg text-body-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
        />
      </div>
              ...
              >
                <div
                  className={cn(
                    ...
                    selectedId === node.id ? 'border-primary-600 bg-primary-600' : 'border-edge-strong'
                  )}
                >
                  {selectedId === node.id && <Check className="w-3 h-3 text-white" />}
                </div>

                <div className="p-1.5 rounded-lg bg-surface-muted">{getNodeIcon(node)}</div>
    ...
    <div>
      ...
      >
        ...
        <div
          className={cn(
            'p-1.5 rounded-lg',
            node.accountCategory === 'ROOT' ? 'bg-primary-800' : 'bg-surface-muted'
          )}
        >
          {getNodeIcon()}
        </div>
...
const ValidationPanel: React.FC<ValidationPanelProps> = ({ validation, loading }) => {
  if (loading) {
    return (
      <div className="flex items-center gap-2 p-4 bg-surface-page rounded-lg">
  ...
  return (
    <div className="space-y-2 p-4 bg-surface-page rounded-lg">
      ...
      <div className="grid grid-cols-2 gap-3 pt-2 border-t border-edge mt-3">
  ...
  return (
    ...
    >
      <div className="-mx-6 -my-6">
        {/* Progress Steps */}
          <div className="px-6 py-3 border-b border-edge-subtle bg-surface-page">
          ...
          {/* Footer */}
          <div className="px-6 py-4 border-t border-edge bg-surface-page flex items-center justify-between">
            <button

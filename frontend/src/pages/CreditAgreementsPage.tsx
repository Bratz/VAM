...
const UtilizationBar: React.FC<{ utilized: number; limit: number; currency: string }> = ({ utilized, limit, currency }) => {
  ...
  const getColor = () => pct >= 90 ? 'bg-error-500' : pct >= 75 ? 'bg-warning-500' : 'bg-success-500';
  return (
  ...
  return (
    <Card className={cn("p-4 hover:shadow-md transition-shadow", isExpiringSoon && agreement.status === 'ACTIVE' && "border-warning-300 bg-warning-50/30")}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
        ...
        {isExpiringSoon && <div className="flex items-center gap-1 text-warning-600 dark:text-warning-300"><AlertTriangle className="w-3 h-3" /><span>{daysToExpiry} days left</span></div>}
      </div>

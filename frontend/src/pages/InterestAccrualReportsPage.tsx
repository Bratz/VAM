...
const StatCard: React.FC<{
  ...
  const colorClasses = {
    primary: 'bg-primary-100 text-primary-600 dark:bg-primary-700 dark:text-primary-200',
    ...
    <Card hover className="p-4">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="text-xl font-bold text-primary-900 mt-1 tracking-tight dark:text-neutral-50">{value}</p>
    ...
    <Page>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 animate-fade-in" style={{ animationDelay: '0.15s' }}>
          <StatCard title="Loan Interest" value={formatCurrency(summary.totalLoanInterestAccrued, summary.currency)} subtitle={`${summary.loanAccrualCount} accruals`} icon={<CreditCard className="w-5 h-5" />} color="error" />
          <StatCard title="Deposit Interest" value={formatCurrency(summary.totalDepositInterestAccrued, summary.currency)} subtitle={`${summary.depositAccrualCount} accruals`} icon={<PiggyBank className="w-5 h-5" />} color="success" />
          <StatCard title="VA Credit" value={formatCurrency(summary.totalVaCreditInterest, summary.currency)} subtitle={`${summary.vaCreditAccrualCount} accruals`} icon={<TrendingUp className="w-5 h-5" />} color="success" />
          <StatCard title="VA Debit" value={formatCurrency(summary.totalVaDebitInterest, summary.currency)} subtitle={`${summary.vaDebitAccrualCount} accruals`} icon={<TrendingDown className="w-5 h-5" />} color="error" />
          <StatCard title="Net Interest Income" value={formatCurrency(summary.netInterestIncome, summary.currency)} subtitle={`Spread: ${formatCurrency(summary.spreadEarned, summary.currency)}`} icon={<BarChart3 className="w-5 h-5" />} color={summary.netInterestIncome >= 0 ? 'success' : 'error'} />
        </div>
      )}

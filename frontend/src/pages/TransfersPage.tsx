      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className={cn(
              'text-xl font-bold',
              hasAmount ? 'text-primary-900 dark:text-neutral-50' : 'text-neutral-300 dark:text-neutral-600'
            )}>
        ...
        <div className="flex items-center gap-2 py-2">
          <div className={cn(
            ...
            canSubmit ? 'bg-success-500' : 'bg-amber-400'
          )} />
...
export default function TransfersPage() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const [accountsRes, corporatesRes, programsRes, transferOutRes, debitRes, poboRes, roboRes, creditRes] = await Promise.all([
                  ...
                  {recentTransfers.map((txn) => {
                    ...
                    const getTransferType = () => {
                      switch (txn.movementType) {
                        case 'TRANSFER_OUT':
                          return { label: 'Internal', variant: 'info' as const, icon: '↔' };
                        case 'DEBIT':
                          return { label: 'Outward', variant: 'primary' as const, icon: '→' };
                        case 'POBO_DEBIT':
                          return { label: 'POBO', variant: 'warning' as const, icon: '⟶' };
                        case 'ROBO_CREDIT':
                          return { label: 'ROBO', variant: 'success' as const, icon: '←' };
                        case 'CREDIT':
                        case 'TRANSFER_IN':
                          return { label: 'Inward', variant: 'success' as const, icon: '←' };
                        case 'EXCEPTION_CREDIT':
                          return { label: 'Exception', variant: 'warning' as const, icon: '⚠' };
                        default:
                          return { label: txn.movementType || 'Transfer', variant: 'neutral' as const, icon: '→' };
                      }
                    };

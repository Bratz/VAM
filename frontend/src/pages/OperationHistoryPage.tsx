...
const Badge: React.FC<{ variant?: 'default' | 'success' | 'warning' | 'error' | 'info' | 'purple' | 'orange'; children: React.ReactNode; size?: 'sm' | 'md' }> = ({ variant = 'default', children, size = 'sm' }) => {
  const variants = {
    ...
    info: 'bg-info-100 text-info-700 dark:bg-info-500/20 dark:text-info-300', purple: 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-300',
...
const getOperationIcon = (type: OperationType) => {
  switch (type) {
    ...
    case 'MOVE_AGGREGATION': return <Folder className="w-5 h-5 text-indigo-600 dark:text-indigo-300" />;
    case 'ACQUISITION': return <Building2 className="w-5 h-5 text-success-600 dark:text-success-300" />;
    case 'MERGER': return <GitMerge className="w-5 h-5 text-purple-600 dark:text-purple-300" />;

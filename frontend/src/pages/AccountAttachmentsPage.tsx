...
const RELATIONSHIP_CONFIG: Record<RelationshipType, { 
  ...
  AUTHORIZED: { label: 'Authorized', icon: UserCheck, color: 'text-purple-600 dark:text-purple-300', bgColor: 'bg-purple-50 dark:bg-purple-500/10', description: 'Authorized to perform transactions within limits' },
  GUARANTOR: { label: 'Guarantor', icon: Shield, color: 'text-amber-600 dark:text-amber-300', bgColor: 'bg-amber-50 dark:bg-amber-500/10', description: 'Guarantees obligations on the account' },
              ...
              filteredNodes.length === 0 ? (
                ...
                filteredNodes.map(node => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn('p-1.5 rounded', node.type === 'GROUP' ? 'bg-primary-900' : 'bg-indigo-100 dark:bg-indigo-500/20')}>
                        <Icon className={cn('w-3 h-3', node.type === 'GROUP' ? 'text-white' : 'text-indigo-700 dark:text-indigo-300')} />
                      </div>
                      <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              filteredEntities.map(entity => (
                <button key={entity.id} type="button" onClick={() => { onSelect(entity.id); setIsOpen(false); }} className={cn('w-full flex items-center gap-3 px-3 py-2 hover:bg-neutral-50 text-left dark:hover:bg-primary-800/50', selectedId === entity.id && 'bg-primary-50 dark:bg-primary-800/40')}>
                  <div className="p-1.5 rounded bg-indigo-100 dark:bg-indigo-500/20"><Building2 className="w-3 h-3 text-indigo-700 dark:text-indigo-300" /></div>
                  <div className="flex-1 min-w-0">
  ...
  return (
    <Page>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: 'Total', value: displayStats.totalAttachments, icon: Link2, iconBg: 'bg-primary-100 dark:bg-primary-700', iconColor: 'text-primary-600 dark:text-primary-200', valueColor: 'text-primary-900 dark:text-neutral-50' },
          { label: 'Active', value: displayStats.activeAttachments, icon: CheckCircle2, iconBg: 'bg-success-100 dark:bg-success-500/20', iconColor: 'text-success-600 dark:text-success-300', valueColor: 'text-success-600 dark:text-success-300' },
          { label: 'Pending', value: displayStats.pendingApproval, icon: Clock, iconBg: 'bg-warning-100 dark:bg-warning-500/20', iconColor: 'text-warning-600 dark:text-warning-300', valueColor: 'text-warning-600 dark:text-warning-300' },
          { label: 'Owner', value: displayStats.ownerRelationships, icon: Key, iconBg: 'bg-info-100 dark:bg-info-500/20', iconColor: 'text-info-600 dark:text-info-300', valueColor: 'text-info-600 dark:text-info-300' },
          { label: 'Authorized', value: displayStats.authorizedRelationships, icon: UserCheck, iconBg: 'bg-accent-100 dark:bg-accent-500/20', iconColor: 'text-accent-600 dark:text-accent-300', valueColor: 'text-accent-600 dark:text-accent-300' },
          { label: 'Collateral', value: displayStats.collateralRelationships, icon: Lock, iconBg: 'bg-error-100 dark:bg-error-500/20', iconColor: 'text-error-600 dark:text-error-300', valueColor: 'text-error-600 dark:text-error-300' },
        ].map((stat, idx) => (
          <Card key={stat.label} hover className="animate-fade-in" style={{ animationDelay: `${0.1 + idx * 0.05}s` }}>
        ...
        {selectedAttachment && (
          <div className="p-4 space-y-6">
            ...
            {selectedAttachment.relationshipType === 'AUTHORIZED' && (
              <div className="bg-purple-50 rounded-lg p-4 dark:bg-purple-500/10">
                <h4 className="text-sm font-medium text-purple-700 mb-3 dark:text-purple-300">Authorization Limits</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-xs text-purple-600 dark:text-purple-300">Max Transaction</p><p className="font-semibold">{selectedAttachment.maxTransactionAmount ? formatCurrency(selectedAttachment.maxTransactionAmount, 'AED') : 'Unlimited'}</p></div>
                  <div><p className="text-xs text-purple-600 dark:text-purple-300">Daily Limit</p><p className="font-semibold">{selectedAttachment.dailyLimit ? formatCurrency(selectedAttachment.dailyLimit, 'AED') : 'Unlimited'}</p></div>
                </div>
              </div>
            )}
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="New Account Attachment" size="lg">
        <div className="p-4 space-y-4">
          ...
          {createForm.relationshipType === 'AUTHORIZED' && (
            <div className="p-4 bg-purple-50 rounded-lg space-y-4 dark:bg-purple-500/10">
              <h4 className="text-sm font-medium text-purple-700 flex items-center gap-2 dark:text-purple-300"><UserCheck className="w-4 h-4" />Authorization Limits</h4>
              <div className="grid grid-cols-2 gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={createForm.requiresDualAuth} onChange={(e) => setCreateForm(p => ({ ...p, requiresDualAuth: e.target.checked }))} className="rounded text-purple-600 dark:text-purple-300" /><span className="text-sm">Require Dual Authorization</span></label>
            </div>
          )}

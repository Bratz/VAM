...
const SPECIAL_VA_CONFIG: Record<VaSpecialType, { 
  ...
  EXCEPTION: {
    ...
    borderColor: 'border-warning-300 dark:border-warning-500/30',
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        >
          ...
          <ChevronRight className="w-5 h-5 text-cat-1/70 dark:text-cat-1 group-hover:text-cat-1 mt-1" />
        </button>
        ...
        >
          ...
          <ChevronRight className="w-5 h-5 text-cat-2/70 dark:text-cat-2 group-hover:text-cat-2 mt-1" />
        </button>
      </div>
    ...
    try {
      ...
      if (formData.enableIhb && formData.owningEntityId) {
        try {
          ...
          toast.success('IHB enabled with auto-sweep rule', { icon: <Landmark className="w-5 h-5 text-primary-600" /> });
        } catch (ihbErr) {
      ...
      toast.success(`Created AGGREGATION: ${response.data?.nodeName || formData.name}`, { duration: 4000, icon: <FolderPlus className="w-5 h-5 text-primary-600" /> });
      onSuccess(response.data?.vaId); onClose();
    } catch (err: any) {
...
const CreateTransactionVaModal: React.FC<CreateTransactionVaModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Created Transaction VA: ${vaNumber}`, { duration: 4000, icon: <CreditCard className="w-5 h-5 text-primary-600" /> });
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <CurrencyFieldWithMirrorHint
              value={formData.currency}
              onChange={(c) => setFormData(prev => ({ ...prev, currency: c }))}
              baseCurrency={parentNode?.currencyCode}
            />
          </div>
          <div>
            ...
            <PurposeSelect
              ...
              onChange={(v) => setFormData(prev => ({ ...prev, accountPurpose: v }))}
            />
          </div>
        </div>
...
const CreateIhbCurrentAccountModal: React.FC<CreateIhbCurrentAccountModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Created IHB Current Account: ${vaNumber}`, { duration: 4000, icon: <Landmark className="w-5 h-5 text-primary-600" /> });
      onSuccess(response?.data?.id);
      onClose();
    } catch (err: any) {
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <Button
            ...
            className="bg-cat-2 hover:bg-cat-2/90"
          >
          ...
          <div className="w-full bg-neutral-200 rounded-full h-2 dark:bg-primary-800">
            <div 
              className={cn(
                ...
                (ihb.utilizationPercent || 0) > 70 ? 'bg-warning-500' : 'bg-success-500'
              )}
...
/** Which node types may have children added under them — shared by the row's
 *  context menu and the panel header's Add Node button. */
const nodeCanHaveChildren = (n: ExtendedHierarchyNode): boolean =>
  n.accountCategory === 'AGGREGATION' || n.accountCategory === 'ROOT' ||
  n.type === 'GROUP' || n.type === 'REGION' || n.type === 'ENTITY';
...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  // Flip the context menu upward when the row sits too close to the viewport
  // bottom — otherwise the menu clips and its items are unreachable.
  ...
  return (
    <div className="select-none group" data-node-id={node.id}>
      <div
        className={cn(
          ...
          highlightId === node.id && 'ring-2 ring-accent-500 bg-accent-50 dark:bg-accent-500/10',
          style.border
        )}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {(() => {
              ...
              // mystery badge; a counted overflow does not.
              const badges: Array<{ key: string; label: string; el: React.ReactNode }> = [];
              if (node.owningEntity) {
                // This badge may shrink (deep indent starves the row) — an
              ...
              if (node.ihb?.enabled) {
                badges.push(node.ihb.isTreasuryCenter
                  ? { key: 'ihb', label: 'Treasury', el: (
                      <Badge key="ihb" variant="warning" size="sm" className="flex items-center gap-1">
                        <Crown className="w-3 h-3" />Treasury
                      </Badge>
                    )}
                  : { key: 'ihb', label: 'IHB', el: (
                      <Badge key="ihb" variant="info" size="sm" className="flex items-center gap-1">
                        <PiggyBank className="w-3 h-3" />IHB
                      </Badge>
                    )});
                if (node.ihb.sweepEnabled) {
                  const freq = node.ihb.sweepFrequency === 'REAL_TIME' ? 'RT' : 'Daily';
                  badges.push({ key: 'sweep-freq', label: `Sweep ${freq}`, el: (
                    <Badge key="sweep-freq" variant="neutral" size="sm" className="flex items-center gap-1">
                      <RefreshCw className="w-3 h-3" />{freq}
                    </Badge>
                  )});
                }
              }
              if (isCurrencyMirror) {
                badges.push({ key: 'mirror', label: 'M-Node', el: (
                  <Badge key="mirror" variant="info" size="sm" className="bg-cyan-100 text-cyan-700 border-cyan-300 dark:bg-cyan-500/20 dark:text-cyan-300">
                    <Coins className="w-3 h-3 mr-1" />M-Node
                  </Badge>
                )});
              }
              if (specialType === 'SETTLEMENT' || specialType === 'EXCEPTION') {
                badges.push({ key: 'special', label: specialConfig.label, el: (
                  <Badge key="special" variant={specialType === 'SETTLEMENT' ? 'info' : 'warning'} size="sm">
                    <SpecialIcon className="w-3 h-3 mr-1" />{specialConfig.label}
                  </Badge>
                )});
              }
              if (node.participatesInSweep) {
                badges.push({ key: 'sweep', label: 'Sweep', el: <Badge key="sweep" variant="warning" size="sm">Sweep</Badge> });
              }
              if (node.participatesInPooling && node.type !== 'VIRTUAL_ACCOUNT') {
                badges.push({ key: 'pool', label: 'Pool', el: <Badge key="pool" variant="info" size="sm">Pool</Badge> });
              }
              const MAX = 3;
              const hidden = badges.slice(MAX);
              return (
                <>
                  {badges.slice(0, MAX).map(b => b.el)}
                  {hidden.length > 0 && (
                    <span title={hidden.map(b => b.label).join(', ')}>
                      <Badge variant="neutral" size="sm">+{hidden.length}</Badge>
                    </span>
                  )}
                </>
              );
        ...
        {/* A mirror is a same-currency VIEW of its sibling subtree — its net
        ...
        {isCurrencyMirror ? (
          ...
          // every row would show a green ▲ (net === consolidated), which reads
          // as fake good news. Render neutral in that case.
          const hasIcActivity =
        ...
        <div className="relative">
          {/* Visible at rest (not hover-only): discoverable for first-time
              users and reachable on touch devices. */}
          <button
            onClick={(e) => {
              e.stopPropagation();
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={isEnabling ? "Enable In-House Banking" : "IHB Settings"} size="lg">
      <div className="p-4 space-y-4 max-h-[80vh] overflow-y-auto">
        ...
        {willBeTreasuryCenter && (
          <div className="p-4 bg-gradient-to-r from-warning-50 to-warning-50 rounded-lg border border-warning-200 dark:border-warning-500/30 dark:from-warning-500/15 dark:to-warning-500/15">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Crown className="w-5 h-5 text-warning-600 dark:text-warning-300" />
                <span className="font-semibold text-warning-900">Treasury Center Rate Configuration</span>
              </div>
              {!showCreateConfig && (
        ...
        {isEnabling && !willBeTreasuryCenter && treasuryRates && (
          <div className="p-4 bg-gradient-to-r from-warning-50 to-warning-50 rounded-lg border border-warning-200 dark:border-warning-500/30 dark:from-warning-500/15 dark:to-warning-500/15">
            <div className="flex items-center gap-2 mb-3">
              <Crown className="w-5 h-5 text-warning-600 dark:text-warning-300" />
              <span className="font-semibold text-warning-900">Treasury Center Offered Rates</span>
...
const TreasuryHierarchyPage: React.FC = () => {
  ...
  // Post-creation focus: node to expand-to/scroll-to/flash once data reloads.
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (hierarchyRes.success && hierarchyRes.data) {
        ...
        // Preserve the user's expansion state across reloads (e.g. after node
        // creation). Keep only ids that still exist; when nothing carries over
        // (first load, or corporate/program switch), fall back to the default.
        setExpandedIds(prev => {
          const allIdSet = new Set(allIds);
          const kept = [...prev].filter(id => allIdSet.has(id));
          return kept.length > 0 ? new Set(kept) : new Set(allIds.slice(0, 10));
        });
      }
  ...
  // Context-menu path: the type is chosen in the menu itself — open the
  // matching modal directly. The header "Add Node" button still routes
  // through the type-selector modal (its cards explain each type).
  const handleAddChild = (parentNode: ExtendedHierarchyNode, type?: NodeCreationType) => {
    setAddNodeParentNode(parentNode);
    if (type) handleNodeTypeSelect(type);
    else setShowAddNodeTypeSelector(true);
  };
  ...
  // After creation: reload, then focus the new node (expand its ancestor
  // chain, scroll to it, flash it) — see the pendingFocusId effect.
  const handleCreationSuccess = (newNodeId?: string) => {
    loadData();
    setAddNodeParentNode(null);
    if (newNodeId) setPendingFocusId(newNodeId);
  };

  // Resolve the pending focus once the refreshed hierarchy is in state.
  useEffect(() => {
    if (!pendingFocusId || !hierarchy) return;
    const ancestors: string[] = [];
    const dfs = (n: ExtendedHierarchyNode, trail: string[]): boolean => {
      if (n.id === pendingFocusId) { ancestors.push(...trail); return true; }
      return (n.children || []).some(c => dfs(c as ExtendedHierarchyNode, [...trail, n.id]));
    };
    const found = dfs(hierarchy, []);
    // Not found = the effect ran against the pre-reload hierarchy (or the node
    // is filtered out) — keep the pending id so the post-reload run resolves it.
    if (!found) return;
    setPendingFocusId(null);
    setExpandedIds(prev => new Set([...prev, ...ancestors]));
    setHighlightNodeId(pendingFocusId);
    setTimeout(() => {
      document.querySelector(`[data-node-id="${pendingFocusId}"]`)
            ...
            <div className="flex items-center justify-between p-4 border-b border-neutral-200 dark:border-primary-800">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  disabled={!selectedNode || !nodeCanHaveChildren(selectedNode)}
                  title={!selectedNode ? 'Select a node first' : !nodeCanHaveChildren(selectedNode) ? 'Selected node cannot have children' : undefined}
                  onClick={() => { if (selectedNode) { setAddNodeParentNode(selectedNode); setShowAddNodeTypeSelector(true); } }}
                >

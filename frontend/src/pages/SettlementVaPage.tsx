  ...
  return (
    <Card padding="md" hover className="cursor-pointer hover:ring-2 hover:ring-primary-200" onClick={() => onView(va)}>
      <div className="flex items-start gap-4">
        <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center', isException ? 'bg-warning-100 dark:bg-warning-500/20' : 'bg-purple-100 dark:bg-purple-500/20')}>
          <Icon className={cn('w-6 h-6', isException ? 'text-warning-700 dark:text-warning-300' : 'text-purple-700 dark:text-purple-300')} />
        </div>
...
const DetailDrawer: React.FC<{ va: SettlementVa | null; detail: SettlementVaDetailResponse | null; loading: boolean; onClose: () => void }> = ({ va, detail, loading, onClose }) => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-xl z-50 flex flex-col dark:bg-primary-900">
      <div className="flex items-center justify-between p-6 border-b">
        <div><h2 className="text-lg font-semibold">{va.vaNumber}</h2><p className="text-sm text-neutral-500 dark:text-neutral-400">{va.vaName}</p></div>
        <button onClick={onClose} className="p-2 hover:bg-neutral-100 rounded-lg dark:hover:bg-primary-800"><X className="w-5 h-5" /></button>
      </div>
      ...
      <div className="p-4 border-t"><Button variant="outline" onClick={onClose}>Close</Button></div>
    </div>
  );
};
...
const SettlementVaPage: React.FC<Props> = ({ programId: propProgramId, onBack }) => {
  ...
  return (
    <Page>
      ...
      {selectedVa && <><div className="fixed inset-0 bg-primary-950/40 z-40" onClick={() => setSelectedVa(null)} /><DetailDrawer va={selectedVa} detail={vaDetail} loading={detailLoading} onClose={() => setSelectedVa(null)} /></>}

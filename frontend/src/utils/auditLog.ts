...
export const auditLog = {
  ...
  async record(
    ...
    // The backend has NO /audit/* controller today, so the previous
    // fire-and-forget POST produced a guaranteed 404 + console noise on
    // every audited action. Flip the `audit.backend` feature flag on when
    // the V2 pipeline lands — the call shape below is ready.
    if (featureFlags.isOn('audit.backend')) {
      try {
        void apiClient

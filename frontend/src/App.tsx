...
const BaaSDashboardPage: React.FC = () => (
  <div className="space-y-6 animate-page-enter">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {[
        { label: 'Active Partners', value: '12', sub: '↑ 2 this month', iconBg: 'bg-primary-100', iconColor: 'text-primary-600', valueColor: 'text-primary-600' },
        { label: 'Total Wallets', value: '17,250', sub: '↑ 1,250 this month', iconBg: 'bg-success-100', iconColor: 'text-success-600', valueColor: 'text-success-600' },
        { label: 'Total Float', value: 'AED 174.5M', sub: '↑ 12% vs last month', iconBg: 'bg-accent-100', iconColor: 'text-accent-600', valueColor: 'text-accent-600' },
        { label: 'Daily Volume', value: 'AED 2.5M', sub: '4,500 transactions', iconBg: 'bg-info-100', iconColor: 'text-info-600', valueColor: 'text-info-600' },
      ].map((stat, i) => (
        <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 hover:shadow-md transition-shadow animate-fade-in" style={{ animationDelay: `${0.05 + i * 0.05}s` }}>
          <div className="flex items-center justify-between mb-3">
            <div className={`w-10 h-10 ${stat.iconBg} rounded-xl flex items-center justify-center`}>
              <TrendingUp className={`w-5 h-5 ${stat.iconColor}`} />
            </div>
          </div>
          <p className="text-sm text-neutral-500">{stat.label}</p>
          <p className={`text-2xl font-bold ${stat.valueColor} mt-1`}>{stat.value}</p>
          <p className="text-xs text-success-600 mt-2">{stat.sub}</p>
        </div>
      ))}
    </div>
...
const BaaSPartnersPage: React.FC = () => (
  <div className="space-y-6 animate-page-enter">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      {[
        { label: 'Total Partners', value: '12', iconBg: 'bg-primary-100', iconColor: 'text-primary-600' },
        { label: 'Active', value: '10', iconBg: 'bg-success-100', iconColor: 'text-success-600' },
        { label: 'Pending Approval', value: '2', iconBg: 'bg-warning-100', iconColor: 'text-warning-600' },
        { label: 'Total Programs', value: '28', iconBg: 'bg-info-100', iconColor: 'text-info-600' },
      ].map((stat, i) => (
        <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 hover:shadow-md transition-shadow animate-fade-in" style={{ animationDelay: `${0.1 + i * 0.05}s` }}>
...
const BaaSCardsPage: React.FC = () => (
  <div className="space-y-6 animate-page-enter">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      {[
        { label: 'Total Cards', value: '5,420', iconBg: 'bg-primary-100', iconColor: 'text-primary-600' },
        { label: 'Active', value: '4,850', iconBg: 'bg-success-100', iconColor: 'text-success-600' },
        { label: 'Suspended', value: '120', iconBg: 'bg-warning-100', iconColor: 'text-warning-600' },
        { label: 'Monthly Spend', value: 'AED 2.5M', iconBg: 'bg-info-100', iconColor: 'text-info-600' },
      ].map((stat, i) => (
        <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 hover:shadow-md transition-shadow animate-fade-in" style={{ animationDelay: `${0.1 + i * 0.05}s` }}>
          <div className={`w-10 h-10 ${stat.iconBg} rounded-xl flex items-center justify-center mb-3`}>
...
const BaaSSettlementsPage: React.FC = () => (
  <div className="space-y-6 animate-page-enter">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      {[
        { label: 'Pending Settlements', value: '8', iconBg: 'bg-warning-100', iconColor: 'text-warning-600' },
        { label: 'Settled Today', value: '12', iconBg: 'bg-success-100', iconColor: 'text-success-600' },
        { label: 'Total Amount', value: 'AED 4.2M', iconBg: 'bg-primary-100', iconColor: 'text-primary-600' },
        { label: 'Partners', value: '10', iconBg: 'bg-info-100', iconColor: 'text-info-600' },
      ].map((stat, i) => (
        <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 hover:shadow-md transition-shadow animate-fade-in" style={{ animationDelay: `${0.05 + i * 0.05}s` }}>
          <div className={`w-10 h-10 ${stat.iconBg} rounded-xl flex items-center justify-center mb-3`}>
            <Wallet className={`w-5 h-5 ${stat.iconColor}`} />
          </div>
          <p className="text-sm text-neutral-500">{stat.label}</p>
          <p className="text-2xl font-bold text-primary-900 mt-1">{stat.value}</p>
        </div>
...
const BaaSTransactionsPage: React.FC = () => (
  <div className="space-y-6 animate-page-enter">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      {[
        { label: 'Today\'s Volume', value: '4,520', iconBg: 'bg-primary-100', iconColor: 'text-primary-600' },
        { label: 'Successful', value: '4,480', iconBg: 'bg-success-100', iconColor: 'text-success-600' },
        { label: 'Failed', value: '40', iconBg: 'bg-error-100', iconColor: 'text-error-600' },
        { label: 'Total Amount', value: 'AED 2.5M', iconBg: 'bg-info-100', iconColor: 'text-info-600' },
      ].map((stat, i) => (
        <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 hover:shadow-md transition-shadow animate-fade-in" style={{ animationDelay: `${0.05 + i * 0.05}s` }}>
          <div className={`w-10 h-10 ${stat.iconBg} rounded-xl flex items-center justify-center mb-3`}>
            <TrendingUp className={`w-5 h-5 ${stat.iconColor}`} />
...
const sectionFor = (page: PageType): SectionKey => {
  ...
  const treasuryPages: PageType[] = ['ihb','pooling','notional-pooling','sweeping','simulator','netting','netting-enhanced','hierarchy',
    'shadow-accounts','multi-bank-liquidity','currency-mirrors','credit-limits','funds-check','balance-aggregation','fx-rates',

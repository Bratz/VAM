...
#### Font Families
```css
--font-sans:    'Bricolage Grotesque', system-ui, sans-serif;  /* body, UI */
--font-display: 'Fraunces', Georgia, ui-serif, serif;           /* page titles, stat values */
--font-mono:    'JetBrains Mono', 'Fira Code', monospace;       /* IBANs, references, code */
```

Distinctive type pair — Bricolage Grotesque (variable, opinionated sans by Mathieu Triay)
paired with Fraunces (variable serif by Undercase Type) for hero headings and big numbers.
Loaded via Google Fonts in `styles/index.css`. See `CLAUDE.md` "Frontend Aesthetics".

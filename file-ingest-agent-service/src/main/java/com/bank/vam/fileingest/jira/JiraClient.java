...
// ponytail: duplicated from defect-fix-service/src/main/java/com/bank/vam/defectfix/jira/JiraClient.java
// (generic Jira Cloud REST plumbing, no defect-specific logic) — extract to a
// shared module if a third consumer needs this. Only change from the
// original: PIPELINE_LABEL and the config type it reads from.
...
public class JiraClient {
    ...
    private final IngestProperties.Jira config;
    ...
    public JiraClient(IngestProperties properties) {
    ...
    /** Creates a ticket labeled with the pipeline label + any extra labels (e.g. domain, dedup). Returns the new issue key. */
    ...
    /**
     * Picks a usable, non-subtask issue type from the project's own configuration — "Bug"/"Task" if
     * the project has one (common, but not guaranteed on a team-managed project), otherwise whatever
    ...
    public record Issue(String key, String summary, String description) {
    }

    /** Oldest pipeline-labeled ticket still in To Do, if any. Not used by step 1 (upload creates and
     * files a ticket synchronously); reserved for the future orchestrator queue (build order step 2+). */
    public Optional<Issue> findOldestOpenTicket() {
        ...
        return Optional.of(new Issue(key, summary, description));
    }

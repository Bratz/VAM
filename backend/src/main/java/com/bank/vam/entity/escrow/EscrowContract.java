...
public class EscrowContract extends BaseEntity {
    ...
    /**
     * Record a fee that has been taken out of the escrow VA. Fees are drawn from
     * the escrowed funds, so the contract balance has to fall with them —
     * otherwise currentBalance drifts above the VA it is supposed to mirror and
     * getRemainingForRelease() reports money that is no longer there.
     */
    public void recordFeeCharged(BigDecimal fee) {
        ...
        this.currentBalance = (this.currentBalance != null ? this.currentBalance : BigDecimal.ZERO).subtract(fee);
    }

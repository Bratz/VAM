...
public class BalanceStructureService {
     ...
     */
    private BigDecimal convertToAED(BigDecimal amount, String currency) {
        if (amount == null) return BigDecimal.ZERO;
        if (currency == null) return amount.setScale(2, RoundingMode.HALF_UP);
        String base = marketProfile.getDefaultCurrency();
        if (currency.equalsIgnoreCase(base)) return amount.setScale(2, RoundingMode.HALF_UP);
        return fxRateService.convert(amount, currency, base).setScale(2, RoundingMode.HALF_UP);
    }
    ...
    private HierarchyNode buildNodeFromVA(VirtualAccount va,
        ...
        BigDecimal consolidated = convertToAED(effectiveBalance, balanceCurrency);
    ...
    private NodeDetail buildNodeDetailFromVA(VirtualAccount va, String reportingCurrency) {
        ...
        return NodeDetail.builder()
            ...
            .consolidatedBalance(convertToAED(currentBalance, va.getCurrencyCode()))

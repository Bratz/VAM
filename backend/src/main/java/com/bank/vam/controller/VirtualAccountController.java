...
public class VirtualAccountController {
    ...
    public ResponseEntity<ApiResponse<VaWithHierarchyResponse>> createVaWithDimensions(
        ...
        // 2. Resolve hierarchy dimensions to find/create the parent node path
        VirtualAccount parentVa = resolveOrCreateHierarchyPath(program, request.getHierarchyDimensions());

        // 3. Create the VA using the resolved parent
        String currency = request.getCurrencyCode() != null
            ? request.getCurrencyCode().toUpperCase()
            : parentVa.getCurrencyCode() != null ? parentVa.getCurrencyCode() : program.getCurrencyCode();

        // 4. Generate VA number
        String vaNumber = generateVaNumber(currency);

        // 5. Build hierarchy path
        String vaCode = request.getVaName().toUpperCase().replaceAll("[^A-Z0-9]", "")
            .substring(0, Math.min(8, request.getVaName().length()));
        String hierarchyPath = parentVa.getHierarchyPathVa() + "/VA-" + vaCode;
        int hierarchyLevel = parentVa.getHierarchyLevel() + 1;

        // 6. Determine account category
        ...
        VirtualAccount va = VirtualAccount.builder()
            ...
            .parentAccountId(parentVa.getId())
        ...
        log.info("Created VA {} under parent {} at level {} using dimensions",
                 va.getVaNumber(), parentVa.getVaNumber(), hierarchyLevel);
        ...
        VaWithHierarchyResponse response = VaWithHierarchyResponse.builder()
            ...
            .parentVaNumber(parentVa.getVaNumber())
    ...
    /**
     * Resolve hierarchy dimensions to find or create the appropriate parent VA.
     * Creates intermediate AGGREGATION nodes as needed.
     */
    private VirtualAccount resolveOrCreateHierarchyPath(Program program, Map<String, String> dimensions) {
        if (dimensions == null || dimensions.isEmpty()) {
            // No dimensions - use program root
            return findOrCreateProgramRootVa(program);
        }

        // Get program root
        VirtualAccount currentParent = findOrCreateProgramRootVa(program);

        // Sort dimensions by level (L1, L2, L3...)
        List<String> sortedLevels = dimensions.keySet().stream()
            .sorted((a, b) -> {
                int levelA = Integer.parseInt(a.replace("L", ""));
                int levelB = Integer.parseInt(b.replace("L", ""));
                return Integer.compare(levelA, levelB);
            })
            .toList();

        // Create/find aggregation nodes for each dimension level
        for (String levelKey : sortedLevels) {
            String dimensionValue = dimensions.get(levelKey);
            if (dimensionValue == null || dimensionValue.isBlank()) {
                continue;
            }

            int levelNumber = Integer.parseInt(levelKey.replace("L", ""));

            // Find existing aggregation node
            VirtualAccount existingNode = findAggregationNode(program.getId(), currentParent.getId(), dimensionValue);

            if (existingNode != null) {
                currentParent = existingNode;
            } else {
                // Create new aggregation node
                currentParent = createAggregationNode(program, currentParent, dimensionValue, levelNumber);
            }
        }
    ...
    private VirtualAccount findOrCreateProgramRootVa(Program program) {
        // Look for existing ROOT VA for this program
        List<VirtualAccount> programVas = virtualAccountRepository.findByProgramId(program.getId());
        Optional<VirtualAccount> rootVa = programVas.stream()
            .filter(va -> va.getAccountCategory() == AccountCategory.ROOT ||
                         (va.getParentAccountId() == null && va.getHierarchyLevel() != null && va.getHierarchyLevel() == 0))
            .findFirst();
        ...
        // Check for any AGGREGATION node at level 1 that can serve as root
        Optional<VirtualAccount> l1Agg = programVas.stream()
            .filter(va -> va.getAccountCategory() == AccountCategory.AGGREGATION &&
                         va.getHierarchyLevel() != null && va.getHierarchyLevel() == 1)
            .findFirst();
        ...
        // Create ROOT VA
        String rootVaNumber = "ROOT-" + program.getProgramCode().toUpperCase();
        String currency = program.getCurrencyCode() != null ? program.getCurrencyCode() : marketProfile.getDefaultCurrency();

        VirtualAccount root = VirtualAccount.builder()
            .vaNumber(rootVaNumber)
            .vaName(program.getProgramName() + " - Root")
            .programId(program.getId())
            .corporateId(program.getCorporateId())
            ...
            .hierarchyLevel(0)
            .hierarchyPathVa("/" + program.getProgramCode())
            .currentBalance(BigDecimal.ZERO)
            .availableBalance(BigDecimal.ZERO)
            .aggregatedBalance(BigDecimal.ZERO)
            .status(VaStatus.ACTIVE)
        ...
        root = virtualAccountRepository.save(root);
        log.info("Created ROOT VA {} for program {}", root.getVaNumber(), program.getProgramCode());
        return root;
    }

    /**
     * Find an existing aggregation node under the parent that matches the dimension value.
     * Also considers Currency Mirrors for CURRENCY dimension at L1.
     */
    private VirtualAccount findAggregationNode(UUID programId, UUID parentId, String dimensionValue) {
        List<VirtualAccount> children = virtualAccountRepository.findByParentAccountId(parentId);

        // First, try to find by exact name match or path segment match (AGGREGATION nodes)
        Optional<VirtualAccount> aggNode = children.stream()
            .filter(va -> (va.getAccountCategory() == AccountCategory.AGGREGATION ||
                          va.getAccountCategory() == AccountCategory.CURRENCY_MIRROR) &&
                         (dimensionValue.equalsIgnoreCase(va.getVaName()) ||
                          dimensionValue.equalsIgnoreCase(va.getCurrencyCode()) ||
                          dimensionValue.equalsIgnoreCase(extractDimensionFromPath(va.getHierarchyPathVa()))))
            .findFirst();
        ...
        // Also check if this is a currency dimension - look for currency mirrors by currency code
        Optional<VirtualAccount> currencyMirror = children.stream()
            .filter(va -> va.getAccountCategory() == AccountCategory.CURRENCY_MIRROR &&
                         dimensionValue.equalsIgnoreCase(va.getCurrencyCode()))
            .findFirst();

        return currencyMirror.orElse(null);
    }

    private String extractDimensionFromPath(String path) {
    ...
    /**
     * Create an aggregation node for the given dimension value.
     * If the dimension looks like a currency code (3 uppercase letters) and it's at L1,
     * creates a CURRENCY_MIRROR instead of a regular AGGREGATION node.
     */
    private VirtualAccount createAggregationNode(Program program, VirtualAccount parent,
                                                  String dimensionValue, int levelNumber) {
        String nodeCode = dimensionValue.toUpperCase().replaceAll("[^A-Z0-9]", "");
        int level = parent.getHierarchyLevel() + 1;

        // Check if this looks like a currency code (3 uppercase letters at L1)
        boolean isCurrencyDimension = levelNumber == 1 &&
                                      dimensionValue.matches("^[A-Z]{3}$") &&
                                      isSupportedCurrency(dimensionValue);

        AccountCategory category;
        String vaNumber;
        String currencyCode;

        if (isCurrencyDimension) {
            // Create as CURRENCY_MIRROR for currency dimensions
            category = AccountCategory.CURRENCY_MIRROR;
            vaNumber = "M-" + dimensionValue + "-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
            currencyCode = dimensionValue; // The dimension value IS the currency
            log.info("Creating CURRENCY_MIRROR for currency {} at level {}", dimensionValue, level);
        } else {
            // Create as regular AGGREGATION node
            category = AccountCategory.AGGREGATION;
            vaNumber = "AGG-" + program.getProgramCode().substring(0, Math.min(4, program.getProgramCode().length())).toUpperCase()
                      + "-L" + levelNumber + "-" + nodeCode;
            currencyCode = parent.getCurrencyCode(); // Inherit from parent
        }

        String hierarchyPath = parent.getHierarchyPathVa() + "/" + nodeCode;

        VirtualAccount aggNode = VirtualAccount.builder()
            .vaNumber(vaNumber)
            .vaName(isCurrencyDimension ? dimensionValue + " Currency Mirror" : dimensionValue)
            .programId(program.getId())
            .corporateId(program.getCorporateId())
            .physicalAccountId(parent.getPhysicalAccountId())
        ...
        aggNode = virtualAccountRepository.save(aggNode);
        log.info("Created {} node {} at level {} for dimension {}",
                 category, aggNode.getVaNumber(), level, dimensionValue);
        return aggNode;
    }
    ...
    private boolean isSupportedCurrency(String code) {
        ...
        return java.util.Set.of(
            "AED", "USD", "EUR", "GBP", "JPY", "CHF", "SGD", "SAR",
            "INR", "CNY", "HKD", "AUD", "CAD", "NZD", "KWD", "BHD",
            "OMR", "QAR", "EGP", "JOD", "LBP", "PKR", "BDT", "LKR"
        ).contains(code.toUpperCase());
    }

    private String generateVaNumber(String currency) {

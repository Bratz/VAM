...
-- Recompute child_count from the actual children. Two sources of drift are being
-- corrected: createExceptionVaIfNotExists incremented child_count without ever
-- inserting a child node (the exception VA hangs off the parent's own node), and
-- the hand-increments read a pre-insert snapshot, so concurrent creation under one
-- parent could lose a count -- silently, since @Version is disabled.
--
-- is_leaf is only forced in one direction: a node that has children is not a leaf.
-- The reverse -- childless implies leaf -- is deliberately NOT applied, because the
-- codebase does not agree with itself about it. Aggregation nodes are created with
-- isLeaf(false) on purpose (they exist to hold children, they just have none yet),
-- while the trigger's delete path sets is_leaf = true when the last child goes. A
-- blanket recompute would silently relabel every empty aggregation node as a leaf,
-- which is a modelling decision, not a data repair. Settling that disagreement is
-- out of scope here.
UPDATE hierarchy_nodes p
SET child_count = actual.n,
    is_leaf = CASE WHEN actual.n > 0 THEN false ELSE p.is_leaf END
FROM (
...
WHERE actual.id = p.id
  AND (p.child_count IS DISTINCT FROM actual.n
       OR (actual.n > 0 AND p.is_leaf IS DISTINCT FROM false));
